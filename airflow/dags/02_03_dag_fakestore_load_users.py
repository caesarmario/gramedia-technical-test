...
## DAG: Load "users" Parquet → PostgreSQL L1 (psycopg2)
####
...
def _run_load(exec_ds: str, **_) -> None:
    ...
    cmd = [
        "python", "-m", f"scripts.load.load_users_parquet_to_l1",
        "--ds", exec_ds,
        "--minio-credentials", minio_str,
        "--pg-credentials", pg_str,
        "--config-file", f"schema_config/fakestore_raw/users_schema_config.json",
        "--target-schema", "l1",
...
with DAG(
    dag_id=f"02_03_dag_fakestore_load_users",
    ...
    t10_params = EmptyOperator(task_id="t10_resolve_params")

    t20_run = PythonOperator(
        task_id="t20_run_load_users",
    ...
    t00_start >> t10_params >> t20_run >> t90_finish